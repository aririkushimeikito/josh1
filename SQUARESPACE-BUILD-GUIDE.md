...
```
HOME | ABOUT | SERVICES ▾ | FAQS | THE ISABELLE EDIT | SHOP | BOOK
```
...
- Nav items: Home · About · Services ▾ (non-linking toggle, no /services page) · FAQs · The Isabelle Edit · Shop (external, new tab) · Book (external, new tab). Group labels AESTHETICS / WELLNESS / SKINCARE inside the dropdown are non-linking. A ready-to-paste HTML/CSS version is in `squarespace-header.html`.
- Shop and Book are plain uppercase text links (per the latest design) opening the Skin Clique shop and booking URLs in a new tab.
...
Domain changes: internal navigation links are root-relative, so they keep working on any custom or temporary domain. Canonical/OG/sitemap use `SITE_URL` (set that env var when rebuilding under a new domain). Squarespace: enter SEO title/description per page under Page Settings → SEO. Set site-wide SEO title format to `%p | Isabelle Joseph, DNP`. Enable SSL, submit `/sitemap.xml` (auto-generated by Squarespace) to Search Console. Pricing is never hard-coded (per blueprint pricing notes).
