...
def header():
    svc = ""
    for gname, items in C.SERVICE_GROUPS:
        svc += f'<li class="dd__group">{esc(gname)}</li>'
...
:root{
  ...
  --wrap:1180px; --gutter:clamp(16px,5vw,64px);
  --radius:2px;
}
...
body{margin:0;background:var(--cream);color:var(--ink);font-family:var(--sans);font-size:17px;line-height:1.65;font-weight:400}
...
.brand__sub{font-size:.62rem;letter-spacing:.16em;text-transform:uppercase;color:var(--ink-muted);margin-top:.25rem}
.nav__list{list-style:none;margin:0;padding:0;display:flex;align-items:center;gap:1.85rem}
.nav__list>li>a{text-decoration:none;font-size:.9rem;font-weight:500;color:var(--brown);letter-spacing:.02em;padding:.5rem 0;display:inline-block}
.nav__list>li>a:not(.btn):hover{color:var(--terracotta-deep)}
.nav__faq{display:none}
...
@media (max-width:960px){
  ...
  .nav__list>li>a{font-family:var(--serif);font-size:1.5rem;padding:.7rem 0;border-bottom:1px solid var(--rule);display:block}
  .nav__faq{display:block}
  ...
  .nav__book{margin-top:1.25rem}
  .nav__book .btn{display:block;text-align:center;padding:1rem}
}
...
@media (max-width:860px){.hero__grid,.hero--service .hero__grid{grid-template-columns:1fr}.hero__media{margin-top:0;order:-1}.hero__media .ph-img{aspect-ratio:4/3}}
...
@media (max-width:860px){.feat-grid--2,.feat-grid--3{grid-template-columns:1fr}}
...
.foot__col a{text-decoration:none;color:var(--brown)}
